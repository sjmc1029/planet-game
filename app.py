...
# 고정된 문제 파장
fixed_wavelengths = [656.234, 656.300, 656.366, 656.254, 656.333]
...
if not st.session_state.quiz_completed:
    ...
    # 랜덤 문제 생성 함수
    def generate_question(target_wavelength):
        angle_options = np.random.choice(range(0, 360, 45), 4, replace=False)
        correct_angle = np.random.choice(angle_options)
        
        # 정답이 선택지에 포함되지 않는 경우 다시 생성
        if correct_angle not in angle_options and not any(opt in multiple_answers.get(correct_angle, []) for opt in angle_options):
            angle_options[0] = correct_angle  # 첫 번째 선택지에 정답 포함
    ...
    if st.session_state.current_question < 5:
        if len(st.session_state.questions) <= st.session_state.current_question:
            target_wavelength = fixed_wavelengths[st.session_state.current_question]
            st.session_state.questions.append(generate_question(target_wavelength))
        ...
        if answer is not None and not st.session_state.answered:
            # 복수 정답 처리
            if answer == question["correct_angle"] or answer in multiple_answers.get(question["correct_angle"], []):
